...
description: "Новый визуальный мир для лендинга, портфолио или редизайна поверхности: направление, дизайн-система, блоки, анти-шаблонность. Подключается ИЗ скилла senior-frontend только при мандате уровня C — когда человек явно просит редизайн или новую поверхность с нуля. НЕ применять к правкам внутри живого интерфейса: скилл несёт свои дефолты по стеку, шрифтам и иконкам и перепишет чужой стайлгайд. Original: anti-slop frontend skill for landing pages, portfolios, and redesigns."
---

<!-- ai-devcontainer:override:start — правка платформы foxford поверх апстрима Leonxlnx/taste-skill -->

## ⚠️ Рамка платформы foxford (перекрывает всё ниже)

Скилл написан для проекта с нуля. Внутри монорепы foxford он подчинён правилам
платформы — `AGENTS.platform.md`, секция «Фронтенд», и скилл `senior-frontend`.

**Когда применяется.** Только при **мандате уровня C**: человек явно попросил
редизайн, новый лендинг или новую поверхность. При уровнях A (обычная правка UI)
и B (полировка в рамках существующей системы) этот скилл **не применяется вовсе** —
там действует правило «нет требования по дизайну, значит требование одно: попасть
в то, что уже есть». Не уверен, какой уровень, — вернись в `senior-frontend`.

**Что ниже не выполняется без явного разрешения человека:**

- **Установка зависимостей.** Все `npm install`, `npx shadcn@latest add`, `yarn add`
  из раздела 2 и Приложения A — справочник, а не инструкция к запуску. Ставить
  пакеты в репозиторий без разрешения запрещено правилом платформы.
- **`npm` и `yarn`.** В репозитории `pnpm` и `nx`. Команды переводи на них.
- **Выбор стека.** Версия Tailwind, библиотека иконок и библиотека анимации
  берутся из `package.json` проекта, а не из дефолтов раздела 3. Миграция
  Tailwind v3 → v4 — отдельная задача, а не побочный эффект правки экрана.
- **Баны шрифтов и иконок** (`Inter` не по умолчанию, `lucide-react` не рекомендован,
  serif «очень не рекомендован») работают при выборе шрифта для **нового** мира.
  Проект, который свой выбор уже сделал, не переобувается ради этого списка.

**Границы поверхности.** «Переделай лендинг» — это лендинг, а не общие токены
репозитория. Правка общих токенов задевает все экраны разом и делается только явно.

**Существующий проект — не чистый лист.** Раздел 11 (Redesign Protocol) и правило
«audit-first» здесь главнее разделов 3–4: сначала вычитай, что в проекте уже есть
(`DESIGN.md` → `SPEC.md` → токены → соседние компоненты), и только потом решай,
что заменяешь.

<!-- ai-devcontainer:override:end -->
...
* **Styling:** версия и способ стилизации берутся **из проекта** (`package.json`,
  `tailwind.config.*`, `postcss.config.*`). Дефолт ниже — только для настоящего
  greenfield; менять стилевую систему живого проекта по этому пункту нельзя.
  **Tailwind v4** (default for greenfield). Tailwind v3 only if the existing project demands it.
...
> **Платформа foxford: это справочник, а не инструкция к запуску.** Ни одну
> команду установки ниже не выполняй сам — предложи её человеку и дождись
> разрешения. Менеджер пакетов в репозитории — `pnpm`, не `npm` и не `yarn`.
