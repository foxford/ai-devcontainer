...
class DependencyScanner:
    ...
    # Встроенная база — это статичный демо-набор популярных CVE, а НЕ живой фид
    # (OSV/NVD/GitHub Advisory). Отсутствие находок не означает отсутствие уязвимостей.
    DATA_SOURCE = (
        "built-in demo dataset (static) — НЕ живой CVE-фид; "
        "для реального аудита подключите OSV/NVD/GitHub Advisory"
    )
    ...
    def generate_report(self, scan_results: Dict[str, Any], format: str = 'text') -> str:
        ...
        report.append(f"Data Source: {scan_results.get('data_source', self.DATA_SOURCE)}")
        report.append("⚠️  Встроенная демо-база CVE — не полагайтесь как на полноценный скан.")
        report.append("")
