...
#
# ЧЕТЫРЕ СЛОЯ (overlay), снизу вверх по приоритету. Правило читается фразой
# «проектное бьёт глобальное, моё бьёт общее» — обе оси упорядочены одинаково:
#   1. глобальный       $PLATFORM_ROOT/mcp/servers.json      (общий,  везде)
#   2. пользовательский /opt/ai-tools/share/mcp/servers.json (личный, везде)
#   3. проектный        <repo>/.agents/mcp.json              (общий,  здесь)
#   4. локальный        <repo>/.agents/mcp.local.json        (личный, здесь)
#
# Перекрытие ПОСЕРВЕРНОЕ и рекурсивное: слой может переопределить у нижнего
# сервера только `env`, оставив `command`/`args` нижними. Массивы заменяются
# целиком (склеивать args бессмысленно). Значение `null` вместо объекта =
# выключить сервер из нижележащего слоя.
#
...
#   • всё из /opt/ai-tools/share/mcp/secrets.env (один на машину);
#   • всё из <repo>/.agents/mcp.secrets.env (KEY=VALUE, файл в .gitignore);
#   • всё из окружения процесса.
# Приоритет: проектный secrets > машинный secrets > окружение. Машинный нужен,
# чтобы токен вроде OBSIDIAN_API_KEY вписывался один раз, а не в каждый проект.
# Нераскрытое `${ИМЯ}` остаётся текстом —
...
# <repo>/.ai-devcontainer/mcp-state — по нему чистятся серверы, уехавшие из
...
# Список того, что разложили мы. Лежит в .ai-devcontainer/ — служебная память
# платформы (см. «Где что лежит в проекте» в bin/adc). Раньше был в .claude/, и
# это било больно: человек, снёсший .claude при проблемах с Claude Code, терял
# state-файл, после чего ветка «.mcp.json есть, а раскладывали его не мы»
# навсегда переставала обновлять раздачу — молча и без внятной причины.
STATE_DIR="$REPO_ROOT/.ai-devcontainer"
STATE_FILE="$STATE_DIR/mcp-state"
STATE_FILE_LEGACY="$REPO_ROOT/.claude/.ai-devcontainer-mcp"
...
# Переезд со старого места делаем ДО чтения: иначе проект, раздававший серверы
# по прошлой схеме, выглядел бы как «раскладывали не мы» — и .mcp.json остался
# бы навсегда замороженным на старом наборе.
if [ -f "$STATE_FILE_LEGACY" ] && [ ! -e "$STATE_FILE" ]; then
  mkdir -p "$STATE_DIR" && mv "$STATE_FILE_LEGACY" "$STATE_FILE" \
    && dim "  состояние переехало: .claude/.ai-devcontainer-mcp → .ai-devcontainer/mcp-state"
fi
