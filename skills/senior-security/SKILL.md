...
description: "Threat modeling и аудит безопасности — STRIDE, OWASP, анализ уязвимостей, криптопаттерны, пентест, безопасная архитектура. Когда оцениваешь поверхность атаки или проверяешь безопасность кода."
triggers:
...
### Automated Pentesting with Strix

`strix` is installed in this environment (`/opt/ai-tools/bin/strix`). It runs
autonomous agents that map the attack surface, attempt exploitation inside a
Docker sandbox, and validate each finding with a working proof of concept —
which is what makes it useful for step 3 above: its output is triaged findings,
not a scanner dump you still have to sift.

**Authorization is the precondition, not a formality.** Run it only against
targets the user owns or is explicitly contracted to test. If the target is a
third-party domain, a production system you cannot identify as the user's, or
anything the user has not named as in scope — stop and ask. Do not infer scope
from a URL appearing in the codebase.

**Two things it needs, both checked at runtime, neither configured by the platform:**

```bash
docker info >/dev/null          # демон обязан отвечать: сендбокс поднимается в нём
export STRIX_LLM="openai/gpt-5" # модель для агентов Strix (свой ключ, не сессия Claude)
export LLM_API_KEY="..."        # можно положить в .agents/mcp.secrets.env проекта
```

Its LLM budget is separate from your own session — the run is billed to that
key, so always bound it.

| Задача | Команда |
|---|---|
| Локальный код, быстрый проход | `strix -t . -m quick -n --max-budget 5` |
| Только изменённое в ветке | `strix -t . --scope-mode diff --diff-base origin/main -n --max-budget 5` |
| Поднятое приложение | `strix -t http://localhost:3000 -m standard --max-budget 15` |
| Прицельно по классу дыр | `strix -t . --instruction "Focus on IDOR and auth bypass" --max-budget 10` |
| Продолжить прерванный прогон | `strix --resume <run-name>` |

**Rules that keep this from going wrong:**

- **Always pass `--max-budget`.** Default `-m deep` with 500 turns per agent
  can burn a lot before it stops on its own.
- **`-n` (non-interactive) for anything scripted** — the default is a TUI that
  never exits, which hangs CI and hangs you.
- **Start at `--scope-mode diff` on an existing codebase.** A full deep scan of
  a mature repo produces more findings than anyone will triage in one sitting.
- **Results go to `./strix_runs/`** — keep it out of git (findings contain
  payloads and sometimes captured credentials).
- **Verify before reporting.** Strix attaches a PoC to each finding; a finding
  whose PoC you did not reproduce is a hypothesis, not a vulnerability. Fold
  confirmed ones into the severity matrix below like any other result.
