...
class HTTPClient:
    ...
    def __init__(self, timeout: float = 30.0, headers: Optional[Dict[str, str]] = None,
        ...
        # Create SSL context
        if not verify_ssl:
            # Осознанный opt-in: проверка TLS-сертификата отключена (возможна MITM-атака).
            # Использовать только против доверенных тестовых стендов с самоподписанными сертификатами.
            print(
                "WARNING: TLS verification disabled (verify_ssl=False) — "
                "соединение уязвимо к MITM, используйте только для доверенных стендов.",
                file=sys.stderr,
            )
