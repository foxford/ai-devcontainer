---
name: "senior-qa"
description: "Генерация тестов для React/Next.js — unit (Jest+RTL), e2e (Playwright), анализ покрытия, моки API через MSW, фикстуры, настройка раннеров. Обязательно: анализ изменений → список юзкейсов → пограничные сценарии → полное покрытие. Когда нужно покрыть код тестами или поднять покрытие."
---
...
## Mandatory Process: Use Cases and Boundary Values

Before writing any tests — always, for every task — go through these four steps. Tests covering only the happy path do not count as sufficient coverage.

**Step 1. Analyze the changes.** Study the diff / task / affected components: which inputs, states, and behavior branches were added or changed.

**Step 2. List the use cases.** Write down every user scenario the change affects — not just the main flow, but alternative ones too (cancel, retry, API error, feature unavailable).

**Step 3. For each use case — list the boundary scenarios.** Typical boundary categories:

- **Numbers and ranges:** min, max, min−1, max+1, zero, negatives, off-by-one at pagination/limit boundaries.
- **Strings:** empty string, single character, max length, length+1, leading/trailing whitespace, unicode/emoji.
- **Collections:** empty list, single item, exactly at the limit, limit+1.
- **Nullable inputs:** `null`, `undefined`, missing field in an API response.
- **States:** loading, error, empty state, races (double click, repeated submit), state transitions.
- **Time and async:** timeouts, slow responses, response arriving after the component unmounted.
- **Permissions and access:** unauthenticated, insufficient permissions, expired session.

**Step 4. Cover everything with tests.** Every use case and every relevant boundary scenario gets at least one test (unit or e2e — per the test pyramid level). In your final output, explicitly show a "use case → boundary scenarios → tests" table so it's visible that nothing was missed.
